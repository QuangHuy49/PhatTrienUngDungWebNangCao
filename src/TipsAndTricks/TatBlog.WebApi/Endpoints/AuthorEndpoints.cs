...
public static class AuthorEndpoints
{
    ...
    {
        var routeGroupBuilder = app.MapGroup("/api/authours");
    ...
    /*private static async Task<IResult> GetPostsByAuthorId(
         ...
         IBlogRepository blogRepository)
     {
        ...
        var pagiantionResult = new PaginationResult<PostDto>(postsList);

        return Results.Ok(pagiantionResult);
     }*/
        ...
        IMapper mapper)
    {
        ...
        {
            return Results.Ok(validationResult.Errors.ToResponse());
        }*/
        ...
        int id, IAuthorRepository authorRepository)
    {
        return await authorRepository.DeleteAuthorAsync(id)
            ? Results.Ok(ApiResponse.Success("Author is deteted",
            HttpStatusCode.NoContent))
            : Results.Ok(ApiResponse.Fail(HttpStatusCode.NotFound,
            "Could not find author"));
    }
